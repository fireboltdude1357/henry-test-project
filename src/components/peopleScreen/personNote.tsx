      ...
      <div>
        ...
        {activeTab === "basic" ? (
          ...
          <TripIdeasView
            ...
            draft={dateIdeaDraft}
            setDraft={setDateIdeaDraft}
            addTripIdea={addTripIdea}
            removeTripIdea={removeTripIdea}
            updateTripIdea={updateTripIdea}
            getUploadUrl={getUploadUrl}
            getSignedUrls={getSignedUrls}
          />
        ) : activeTab === "settings" ? (
...
function TripIdeasView({
  ...
  draft: DateIdeaDraft;
  setDraft: React.Dispatch<React.SetStateAction<DateIdeaDraft>>;
  addTripIdea: AddDateIdeaFn;
  removeTripIdea: RemoveDateIdeaFn;
  updateTripIdea: (args: {
    personId: Id<"people">;
    index: number;
    title?: string;
  ...
  // Reuse DateIdeasView with different data source: tripIdeas
  const ideas = ((personData as unknown as { tripIdeas?: DateIdea[] })
    .tripIdeas || []) as DateIdea[];
  const [isDragging, setIsDragging] = useState(false);
  const [editingIdx, setEditingIdx] = useState<number | null>(null);
  const [editTitle, setEditTitle] = useState("");
  const [editNotes, setEditNotes] = useState("");
  ...
  // Render by copying the DateIdeasView JSX, but using the locals above
  // For brevity, reuse the same structure/labels
  return (
    <div style={{ display: "grid", gap: 12 }}>
      <div
        style={{
                ...
                {editingIdx === idx ? (
                  ...
                  <div>
                    <div style={{ display: "flex", alignItems: "center" }}>
                      ...
                      <button
                        onClick={() => removeTripIdea({ personId, index: idx })}
                        style={{
                          padding: "6px 10px",
                          borderRadius: 8,
                          border: "1px solid var(--border)",
